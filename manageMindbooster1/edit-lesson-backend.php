...
if (isset($_POST['edit_lesson'])) {
    ...
    $l_created_date = $_POST['l_created_date'];
    ...
    $sql = "UPDATE tb_lessons SET l_name = ?, l_level = ?, l_lesson_desc = ?, l_lesson_plan = ?, l_worksheet = ?, l_exercise = ?, l_supplementary_note = ?, l_quiz = ?, l_image = ?, l_created_date = ?, l_released_date = ? WHERE l_id = ?";
    $stmt = mysqli_prepare($conn, $sql);

    if ($stmt) {
        mysqli_stmt_bind_param($stmt, "sssssssssssi", $l_name, $l_level, $l_lesson_desc, $l_lesson_plan, $l_worksheet, $l_exercise, $l_supplementary_note, $l_quiz, $l_image, $l_created_date, $l_released_date, $l_id);
