                ...
                <div class="row">
                    <div class="container-fluid">
                        <div class="row">
                            ...
                            <div class="col-md-12 offset-lg-0  p-5 container" style="background: url('../assets/community/pngfiles/Mask group-2.png'); background-repeat: no-repeat; background-position: right; background-size: 34%; ">
                                <img src="../assets/community/pngfiles/Group 299.png" class="img-fluid px-5 ">
                                <p style="text-align: left;" class="px-0">
                                    <?php
                                    $loremText = "
                                    Unleash your creative spark us.Join our vibrant community of creators and unlock the full potential of your.Together,let's embark on an extraordinary journey of self-expression,inspiration,and endless artistic  discovery.Embrace the power within you and let your creative spark illuminnate the word";
                                ...
                                <a  class="btn-sm px-5" data-toggle="modal" data-target="#globalModal"><img src="../assets/community/pngfiles/Group 411.png" alt="Video_thumbnail" style="border-radius: 0;" class="btn btn-sm"></a>
                            </div>
                ...
                <div class="accordion" id="accordionExample">
                    ...
                    while ($communityRow = mysqli_fetch_assoc($communityResult)) {
                        ...
                        while ($categoryRow = mysqli_fetch_assoc($categoryResult)) {
                            ...
                            echo '<a href="community-article.php?community_id=' . $community_id . '&category_id=' . $category_id . '" class="text-secondary"> ' . $categoryName . '<br></a>';
                        }
                        echo '</div>';
                        echo '</div>';
                        echo '</div>';
                ...
                <ul class="nav nav-tabs justify-content-center" id="categoryTabs">
                    ...
                    while ($categoryRow = mysqli_fetch_assoc($categoryResult)) {
                        ...
                        if ($count <= 5) {
                            ...
                            $categoryTabs .= '<a class="nav-link text-uppercase fw-bolder' . ($tab_status) . '" id="category-tab-' . $count . '" data-bs-toggle="pill" href="#category-' . $count . '">' . $categoryName . '</a>';
                            $categoryTabs .= '</li>';
                        }
                        $count++;
                    }
                        ...
                        if (mysqli_num_rows($articleResult) > 0) {
                            ...
                            while ($articleRow = mysqli_fetch_assoc($articleResult)) {
                                ...
                                if (!empty($articleRow['article_thumbnail'])) {
                                    $thumbnailPath = "../assets/community/thumbnails/" . $articleRow['article_thumbnail'];
                                } else {
                                    $thumbnailPath = "../assets/community/thumbnails/default-thumbnail.jpg";
                                }
                                
                                echo '<img src="' . $thumbnailPath . '" class="card-img-top" alt="Article Thumbnail"  style=" border-radius: 10px; height:300px;">';
                                
                                echo '<div class="card-body">';
                                echo '<div class="row like-dislike-buttons">';
                                echo '<div class="col-12 d-flex justify-content-between">';
                                echo '<div>';
                                echo '<button class="like-button text-success"  data-toggle="modal" data-target="#globalModal"><i class="fa fa-thumbs-up"></i> <span id="a_likes_' . $articleRow['article_id'] . '">' . $articleRow['likes'] . '</span></button>';
                                echo '<button class="dislike-button text-danger" data-toggle="modal" data-target="#globalModal"><i class="fa fa-thumbs-down"></i> <span id="a_dislikes_' . $articleRow['article_id'] . '">' . $articleRow['dislikes'] . '</span></button>';
                                echo '</div>';
                                echo '<div class="text-end position-relative">';
                                echo '  <div class="dropdown dropup">';
                                echo '    <button class="btn btn-link " data-toggle="modal" data-target="#globalModal" type="button"   aria-haspopup="true" aria-expanded="false">';
                                ...
                                echo '<a href="view_comments.php?article_id=' . $articleRow['article_id'] . '&category_id=' . $categoryRow['category_id'] . '&community_id=' . $community_id . '" class="text-black">
                                <h5 class="card-title text-dark" style="transition: text-decoration 0.3s;">
                                    ' . $articleRow['ca_title'] . '
                                </h5>
                            </a>';
...
<script>
    var bala = document.getElementById("button");
    var amir = document.getElementById("accordionExample");
    ...
    document.addEventListener("click", function(even) {
        if (!bala.contains(even.target) && !amir.contains(even.target)) {
