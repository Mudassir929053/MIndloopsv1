...
<main id="main">
    <div class="container">
        ...
        $forumQuery = "SELECT * FROM `forum` WHERE `forum_id` = $forumId and published=1";
        ...
        if ($forumResult->num_rows > 0) {
            ...
            $subtopicsQuery = "SELECT * FROM `forum_topic` WHERE `forum_id` = $forumId and published=1";
            ...
            echo '<a href="addTopic.php" data-toggle="modal" data-target="#globalModal" class="btn btn-warning btn-sm  mx-2"><i class="bi bi-plus-circle"></i> Add Article</a>';
            echo '<br>';
            echo '<br>'; ?>
            <div class="row">
                <?php
                ...
                if ($subtopicsResult->num_rows > 0) {
                    while ($subtopicRow = $subtopicsResult->fetch_assoc()) {
                        ...
                        echo '<img class="mb-4 thumbnail" src="../assets/forum/article_thumbnail/' . $subtopicRow['article_thumbnail'] . '" alt="Thumbnail" class="thumbnail">';
                        echo '<h5><i class="fa fa-newspaper-o"></i><a href="forum_topics.php" data-toggle="modal" data-target="#globalModal">' . $subtopicRow['ft_name'] . '</a></h5>';
                        echo '<p><strong>' . date('d F Y', strtotime($subtopicRow['ft_created_at'])) . '</strong></p>';
                        echo '</div>';
                        echo '</div>';
                        $count++;
                    }
