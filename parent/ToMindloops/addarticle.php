...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['article_title']) && isset($_POST['article_description'])) {
        ...
        $sql = "INSERT INTO to_mindloops_articles (article_title, topic_id, article_content, created_by, article_thumbnail, attachment) VALUES ('$articleTitle', '$to_mindloops_id', '$articleDesc', '$user', '$thumbnailName', '$attachmentName')";
        ...
        if ($result) {
            ...
            $updateSql = "UPDATE to_mindloops_articles SET thumbnail_filename = '$thumbnailName', attachment_filename = '$attachmentName' WHERE article_id = '$articleId'";
            mysqli_query($conn, $updateSql);
            echo "<script>
		location.href = 'to-mindloops-article.php?to_mindloops_id=$to_mindloops_id';</script>";
