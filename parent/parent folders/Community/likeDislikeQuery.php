...
if (!empty($likeForumArticle)) {
    ...
    $sqlupdate = "UPDATE community_article SET likes='$total_likes',dislikes='$total_dislikes',liked_by='$likes_str',disliked_by='$dislikes_str' where article_id='$article_id'";
...
if (!empty($likeForumComment)) {
    $sql = "SELECT * FROM `communityarticlecomment` where comment_id='$comment_id'";
    $likes = [];
    $dislikes = [];
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
    ...
    $dislikes_str = implode(',', $dislikes);
    $sqlupdate = "UPDATE communityarticlecomment SET likes='$total_likes',dislikes='$total_dislikes',liked_by='$likes_str',disliked_by='$dislikes_str' where comment_id='$comment_id'";
    if ($action == 'like' && !$liked) {
        $update = $conn->query($sqlupdate);
    } else if ($action == 'dislike' && !$disliked) {
