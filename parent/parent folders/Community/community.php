...
<main id="main">
    <div class="container-fluid mt-5">
        <div class="row">
            ...
            <div class="col-md-10">
                <div>
                    <h1 class="text-danger">Communities</h1>
                    <p>
                ...
                <div class="container-fluid mb-5">
                    <input type="text" class="form-control" id="searchInput" placeholder="Search Community Name">
                    <div class="table-responsive">
                        <table class="table align-middle">
                            ...
                            <tbody id="communityTable">
                                ...
                                $communityQuery = "SELECT * FROM `community` WHERE published = 1";
                                ...
                                while ($communityRow = mysqli_fetch_assoc($communityResult)) {
                                    $communityId = $communityRow['community_id'];
                                    $communityName = $communityRow['community_name'];
                                    $communityDescription = $communityRow['community_description'];
                                    $created_at = new DateTime($communityRow['created_at']);
                                    // Fetch the count of categories for the current community
                                    $categoryQuery = "SELECT `category_id`, COUNT(`category_id`) AS `category_count` FROM `community_category` WHERE `community_id` = $communityId";
                                    $categoryResult = mysqli_query($conn, $categoryQuery);
                                    $categoryRow = mysqli_fetch_assoc($categoryResult);
                                    $categoryCount = $categoryRow['category_count'];
                                    $category_idd = $categoryRow['category_id'];
                                    ...
                                    echo '<td>';
                                    if ($categoryCount > 0) {
                                        echo '<a href="community-article.php?community_id=' . $communityId . '" class="text-warning">' . $communityName . '</a>';
                                    } else {
                                        echo '<span class="text-warning">' . $communityName . '</span>';
                                    }
                                    echo '</td>';
                                    ...
                                    if ($categoryCount > 0) {
                                        echo '<a href="community-article.php?community_id=' . $communityId . '&category_id='.$category_idd.'"><span class="badge bg-primary rounded-pill">View Community</span></a>';
                                    } else {
                                        echo '<span class="badge bg-primary rounded-pill disabled">View Community</span>';
                                    }
                                    echo '</td>';
                                    echo '</tr>';
