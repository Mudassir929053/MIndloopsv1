                                    ...
                                    <div id="<?php echo $category; ?>" class="tab-pane fade<?php echo $key == $cat_pos ? ' show active' : ''; ?>">
                                        <div class="row search-results">
                                            ...
                                            if (mysqli_num_rows($result) > 0) {
                                                ...
                                                while ($row = mysqli_fetch_assoc($result)) {
                                                    $itemCount++;
                                                    $isPremium = ($page == 1 && $itemCount <= 2) ? 'not-premium' : 'premium blur-text no-click'; // Check if item is premium or not
                                                    $isPremium1 = ($page == 1 && $itemCount <= 2) ? 'not-premium' : 'premium'; // Check if item is premium or not
                                                    if ($i % 3 == 0 && $i != 0) {
                                                    ...
                                                    echo '<img src="' . $row['c_file'] . '" alt="" class="img-fluid mindbooster-img ' . $isPremium . '">';
                                                    echo '<span class="text-danger ' . $isPremium . '">Living Skills</span>';
                                                    echo '<h4 class="text-black ' . $isPremium . '">' . $row['c_title'] . '</h4>';
                                                    echo '<span class="text-secondary ' . $isPremium . '">' . date('j F Y', strtotime($row['c_date'])) . '</span>';
                                                    // echo '<p class="text-black ' . $isPremium . '">' . $row['c_desc'] . '</p>';
                                                    $c_desc = $row['c_desc'];
                                                    if (strlen($c_desc) > 100) {
                                                        $c_desc = substr($c_desc, 0, 100) . "...";
                                                        echo '<p class="text-black ' . $isPremium . '">' . $c_desc . ' <a href="#" class="text-danger">(description)</a></p>';
                                                    } else {
                                                        echo '<p class="text-black ' . $isPremium . '">' . $c_desc . '</p>';
                                                    }
                                                    echo '<div class="mt-auto">';
                                                    echo '<div class="row align-items-end">';
                                                    echo '<div class="col-md-1 text-start">';
                                                    echo '<a href="https://wa.me/?text=' . urlencode($row['c_title'] . ' - ' . $row['c_desc'] . ' - ' . 'https://192.168.0.108/mindbooster-description.php?mb=' . $row['c_id']) . '" class="text-end text-danger ' . $isPremium . ' d-block mt-3">';
                                                    echo '<img src="https://img.icons8.com/color/30/null/whatsapp--v1.png"/></a>';
                                                    echo '</div>';
                                                    echo '<div class="col-md-1">';
                                                    echo '<a href="https://www.facebook.com/sharer/sharer.php?u=' . urlencode('https://192.168.0.108/mindbooster-description.php?mb=' . $row['c_id']) . '" class="text-end text-danger ' . $isPremium . ' d-block mt-3">';
                                                    echo '<img src="https://img.icons8.com/fluency/30/null/facebook-new.png"/></a>';
                                                    echo '</div>';
                                                    echo '<div class="col-md-10">';
                                                    echo '<a href="mindbooster-description.php?mb=' . $row['c_id'] . '" class="text-end text-danger ' . $isPremium . ' d-block mt-3">';
                                                    echo 'Read more</a>';
                                                    echo '</div>';
                                                    echo '</div>';
                                                    echo '</div>';
