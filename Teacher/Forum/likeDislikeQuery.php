...
if(!empty($likeForumArticle)){
    // var_dump($_REQUEST);
    // exit;
    $sql = "SELECT * from forum_topic where forumtopic_id='$article_id'";
    $likes=[];
    $dislikes = [];
    ...
    $sqlupdate = "UPDATE forum_topic SET likes='$total_likes',dislikes='$total_dislikes',liked_by='$likes_str',disliked_by='$dislikes_str' where forumtopic_id='$article_id'";
...
if(!empty($likeForumComment)){
    $sql = "SELECT * FROM `forum_postcomment` where comment_id='$comment_id'";
    $likes=[];
    $dislikes = [];
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
    ...
    $dislikes_str = implode(',',$dislikes);
    $sqlupdate = "UPDATE forum_postcomment SET likes='$total_likes',dislikes='$total_dislikes',liked_by='$likes_str',disliked_by='$dislikes_str' where comment_id='$comment_id'";
    if($action=='like' && !$liked){
        $update = $conn->query($sqlupdate);
    }
    else if($action == 'dislike' && !$disliked){
