...
<?php include '../commonPHP/dbconnect.php'; ?>
<?php
if (!session_id()) {
    session_start();
}
...
<main id="main">
    <div id="portfolio-details">
        <div class="container-fluid">
            <div class="row">
                <!-- <img src="../../assets/mindbooster/MB Banner.png" alt="" style="padding:0;" width="100%"> -->
    ...
    <div class="container-fluid">
        <div class="container py-5">
            <div class="row">
                ...
                $query = "SELECT DISTINCT ts_type FROM  tb_teachsupport ";
                        ...
                        <div class="container py-5">
                            <ul class="nav nav-tabs fs-4 justify-content-center">
                                ...
                                foreach ($categories as $key => $category) {
                                    ...
                                    echo '<a class="nav-link text-uppercase fw-bolder' . ($key == $cat_pos ? ' active' : '') . '" data-bs-toggle="tab" href="#' . $category . '">' . $categoryFullNames[$category] . '</a>';
                                    echo '</li>';
                                }
                                ?>
                            </ul>
                            ...
                            <div class="tab-content pt-3">
                                ...
                                foreach ($categories as $key => $category) {
                                    ...
                                    $offset = ($page - 1) * $limit;
                                    $query = "SELECT DISTINCT * FROM  tb_teachsupport WHERE ts_type = '$category'";
                                    $result = mysqli_query($conn, $query);
                                    if (!$result) {
                                        echo "Error executing query: " . mysqli_error($conn);
                                        continue;
                                    ...
                                    $query = "SELECT * FROM `tb_teachsupport` WHERE ts_type = '$category' LIMIT $offset, $limit";
                                    $result = mysqli_query($conn, $query);
                                    if (!$result) {
                                        echo "Error executing query: " . mysqli_error($conn);
                                        continue;
                                        ...
                                        <div class="row">
                                            ...
                                            if (mysqli_num_rows($result) > 0) {
                                                ...
                                                while ($row = mysqli_fetch_assoc($result)) {
                                                    ...
                                                    echo '<img src="../' . $row['ts_filePath'] . '" alt="" class="img-fluid mindbooster-img">';
                                                    echo '<span class="text-danger">Living Skills</span>';
                                                    echo '<h4 class="text-black">' . $row['ts_title'] . '</h4>';
                                                    echo '<span class="text-secondary">' . date('j F Y', strtotime($row['ts_date'])) . '</span>';
                                                    echo '<p class="text-black">' . $row['ts_desc'] . '</p>';
                                                    echo '<div class="mt-auto">';
                                                    echo '<div class="row align-items-end">';
                                                    echo '<div class="col-md-1 text-start">';
                                                    echo '<a href="https://wa.me/?text=' . urlencode($row['ts_title'] . ' - ' . $row['ts_desc'] . ' - ' . 'https://192.168.0.108/mindbooster-description.php?ts=' . $row['ts_id']) . '" class="text-end text-danger d-block mt-3">';
                                                    echo '<img src="https://img.icons8.com/color/30/null/whatsapp--v1.png"/></a>';
                                                    echo '</div>';
                                                    echo '<div class="col-md-1">';
                                                    echo '<a href="https://www.facebook.com/sharer/sharer.php?u=' . urlencode('https://192.168.0.108/mindbooster-description.php?mb=' . $row['ts_id']) . '" class="text-end text-danger d-block mt-3">';
                                                    echo '<img src="https://img.icons8.com/fluency/30/null/facebook-new.png"/></a>';
                                                    echo '</div>';
                                                    echo '<div class="col-md-10">';
                                                    echo '<a href="mindbooster-description.php?mb=' . $row['ts_id'] . '" class="text-end text-danger d-block mt-3">';
                                                    echo 'Read more</a>';
                                                    echo '</div>';
                                                    echo '</div>';
