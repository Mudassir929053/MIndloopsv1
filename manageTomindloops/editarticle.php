...
$sql = "SELECT * FROM to_mindloops_articles where article_id = '$article_id'";
$data=[];
        ...
        <a href="tomindloopsArticles.php?topic_id=<?= $topic_id ?>" class="btn btn-warning col-lg-1"><i class="bi bi-arrow-left"></i> Back</a>
          ...
          <form id="insertLpForm" action="#" onsubmit="return updateArticle(this)" method="POST" enctype='multipart/form-data'>
            <div class="form-group"> 
                ...
                <input type="text" class="form-control" id="ca_name" name="ca_name" value="<?= $data['article_title']?>" placeholder="Enter Title" required>
                <input type="hidden" class="form-control" id="article_id" name="article_id" value="<?= $data['article_id'] ?>">
                <input type="hidden" class="form-control" id="topic_id" name="topic_id" value="<?= $topic_id ?>">
            </div>
            <br>
            <div class="form-group"> 
                <label for="c_desc">Article Description</label>
                <textarea class="form-control" id="summernote" name="ca_desc" placeholder="Enter Community description..." maxlength="1000" required><?= $data['article_content']?></textarea>
            </div>
            <br>  
  ...
  function updateArticle(obj){
        ...
        fetch(url,{
            ...
            if(data=='Updated successfully'){
                window.location.href = 'tomindloopsArticles.php?topic_id=<?= $topic_id ?>';
            }
        });
        return false;
